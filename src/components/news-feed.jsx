...
export default function NewsFeed() {
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 text-black min-h-screen flex flex-col items-center justify-start pt-24 px-6 bg-white">
      {/* Title */}
      <motion.h2
        className="text-4xl font-bold md:text-6xl leading-tight mb-6 text-black "
        initial={{ opacity: 0, x: -50 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 1 }}
      >
        <span className="text-black-500">Real Time Stock</span>  <span className="text-red-500">Sentiment Analysis</span> 
      </motion.h2>

      <p className="text-center text-lg mb-6 text-black-600 max-w-4xl">
        Real-Time News Analyzer scans financial news and earnings reports to detect market trends. 
        Enter a stock name and get instant bullish or bearish signals for smarter investing!
      </p>
      ...
      <div className="mb-6 flex gap-4 w-full">
        <input
          ...
          value={ticker}
          onChange={(e) => setTicker(e.target.value.toUpperCase())}
          className="px-4 py-2 rounded-lg bg-gray-200 text-black border border-gray-400 w-full sm:w-3/4"
          placeholder="Enter stock symbol (e.g., AAPL)"
        />
        <button
          className="px-6 py-2 bg-blue-600 rounded-lg text-white hover:bg-blue-700 sm:w-1/4"
          onClick={fetchNews}
        >
          Analyze Sentiments
        </button>
      ...
      {sentimentSummary && !loading && (
        <div className="mb-6 w-full flex gap-8">
          {/* Sentiment Analysis Bar Chart */}
          <div className="w-full sm:w-1/3" style={{ height: "300px" }}>
            <h3 className="text-xl font-semibold mb-4 text-center text-black">Sentiment Analysis</h3>
            <Bar
              data={sentimentData}
              options={{
                responsive: true,
          ...
          <div className="w-full sm:w-2/3 space-y-6">
            {news.length > 0 ? (
              news.map((article, index) => (
                <div key={index} className="p-6 bg-gray-100 rounded-lg shadow-lg hover:bg-gray-200 transition duration-300 ease-in-out">
                  <h4 className="text-2xl font-bold mb-3 text-black">{article.title}</h4>
